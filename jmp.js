	...
	{
		airejeu=terrain();
		/*L'ordinateur vérifie s'il peut dejà gagner*/
		var casejoue=pasVictoire(airejeu,'O');
		/*Si ce n'est pas le cas il vérifie si le joueur peut dejà gagner*/
		if(casejoue==-1) casejoue=pasVictoire(airejeu,'X');
			/*Si personne ne peut dejà gagner il joue dans n'importe quelle case*/
			if(casejoue==-1)
			{
				for(var i=0;i!=1;) 
				{
					casejoue=Math.round(Math.random()*8);
					if(airejeu[casejoue]==" ") i=1;
				}
			}
		return casejoue;
	}
	...
	function afficheVainqueur(vainqueur) 
	{
		...
		var phrase1="Bravo! Vous m'avez gagné!"
		var phrase2="Je vous ai gagné!";
	...
	{
			document.tictac.tourjeu.value="Joueur "+(joueur+1);
			document.tictac.tourjeu.id="joueurinfo"+(joueur+1);
	}
